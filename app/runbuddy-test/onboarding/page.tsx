...
export default function RunBuddyTestOnboarding() {
  ...
  function nextStep() {
    ...
    // Step 5 with 0 strength → skip gym question and go straight to plan
    if (step === 5 && strengthDays === 0) {
      saveThenRedirect(0, false)
      return
    }

    // Last step → save and go
    if (step === displayMax) {
      saveThenRedirect(strengthDays ?? 0, hasGym ?? false)
      return
    }
  ...
  function saveThenRedirect(sd: number, gym: boolean) {
    const config = {
      raceDate,
    ...
    // Store in sessionStorage — the test plan page reads it from there
    sessionStorage.setItem('runbuddy-test-config', JSON.stringify(config))
